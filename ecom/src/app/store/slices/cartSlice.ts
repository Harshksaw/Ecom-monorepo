...
const cartSlice = createSlice({
  ...
  reducers: {
    ...
    updateCartItemQuantity: (
      ...
      if (itemIndex >= 0) {
        ...
        const updatedQuantity = Math.max(
          1,
          Math.min(quantity, state.items[itemIndex].stock)
        );
    ...
    loadCartFromStorage: (state) => {
      ...
      try {
        ...
        if (savedCart) {
          ...
          state.deliveryOptions = parsedCart.deliveryOptions || [];
          state.selectedDeliveryOption =
            parsedCart.selectedDeliveryOption || null;
        }
      } catch (error) {
    ...
    setSelectedDeliveryOption: (
      state,
      action: PayloadAction<DeliveryOption>
    ) => {
...
export const selectDeliveryOptions = (state: { cart: CartState }) =>
  state.cart.deliveryOptions;
export const selectSelectedDeliveryOption = (state: { cart: CartState }) =>
  state.cart.selectedDeliveryOption;

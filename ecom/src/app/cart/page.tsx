...
import { CartItem, UserProfile, Address, DeliveryOption } from "./types";
...
const CartPage = () => {
  ...
  const selectedDeliveryOption = useSelector(selectSelectedDeliveryOption);
  const { token } = useAuth();
  ...
  useEffect(() => {
    if (cartItems.length > 0) {
      ...
      dispatch(setSelectedDeliveryOption(options[0] || null));
    }
  }, [cartItems]);
        ...
        if (userInfo) {
          ...
          if (parsedUser && parsedUser.id) {
            ...
            const response = await axios.get(
              `${API_URL}/auth/profile/${parsedUser.id}`
            );
              ...
              if (userData && userData.user) {
                ...
                if (
                  ...
                  setDefaultShippingAddress(
                    defaultShipping || firstShipping || null
                  );
                }
              }
            }
  ...
  useEffect(() => {
    ...
    const shippingValue = selectedDeliveryOption
  ...
  const isCheckoutReady = (): boolean => {
    ...
    if (!userProfile || !userProfile.firstName || !userProfile.email)
      return false;
  ...
  const handleCheckout = async () => {
    ...
    try {
      ...
      if (selectedCurrency !== "INR") {
        toast.success(
          `Your card will be charged in INR (${formatPrice(total)}).`,
          { duration: 6000 }
        );
      }
        ...
        <div className="py-10">
          ...
          <div className="flex flex-col lg:flex-row gap-8">
            <div className="flex-grow">
              <CartItemsList cartItems={cartItems} dispatch={dispatch} />
              <div className="mt-8">
                <Link
                  href="/"
            ...
            <div className="lg:w-96 flex-shrink-0 space-y-6">
              ...
              <OrderSummary
                ...
                onDeliveryOptionChange={(opt) =>
                  dispatch(setSelectedDeliveryOption(opt))
                }
              />
            </div>
          </div>
...
//       const options = cartItems[0]?.deliveryOptions;
//       setDeliveryOptions(options);
//       setSelectedDeliveryOption(options[0] || null);
//     }
//   }, [cartItems]);
...
//       if (selectedCurrency !== "INR") {
//         toast.success(
//           `Your card will be charged in INR (${formatPrice(
//             total
//           )} converts to approximately ₹${total.toLocaleString()}).`,
//           {
//             duration: 6000,
//           }
//         );
